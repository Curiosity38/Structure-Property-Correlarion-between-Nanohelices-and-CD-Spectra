...
'''
sns.set(style="white", palette="Set2")  # 设置为 white 风格，纯白背景
x_sequence = np.linspace(400, 800, 120)
for index in range(0, 800):
    plt.figure(figsize=(10, 6))
    plt.rcParams['axes.linewidth'] = 5
    sns.lineplot(x=x_sequence, y=y_train[index], linewidth=5, color='black')
    ...
    plt.xlabel('Wavelength Index', fontweight='bold')
    ...
    #ax.spines['bottom'].set_linewidth(2)  # 设置坐标轴边框宽度
    ...
    # 保存图片并关闭图形
    plt.savefig(os.path.join('train_vis_wjs', f'CD_{index}.png'))  # 设置紧凑的边界框
    plt.close()
'''
'''
for index in range(0, 600):
    sns.lineplot(x=range(len(y_test[index])), y=y_test[index], label='Actual Spectrum')
    plt.xlabel('Wavelength Index')
    plt.ylabel('Intensity')
    plt.title(f'Actual {index} Spectra')
    plt.legend()
    plt.savefig(os.path.join('test_vis_wjs', f'CD_{index}.png'))
    plt.close()
'''
...
plt.scatter(X_tsne[:len(X_train), 0], X_tsne[:len(X_train), 1], marker='o', c='b', label='X_train', alpha=0.5)
plt.scatter(X_tsne[len(X_train):, 0], X_tsne[len(X_train):, 1], marker='o', c='r', label='X_test', alpha=0.5)
plt.xlabel('t-SNE Component 1')
plt.ylabel('t-SNE Component 2')
plt.title('t-SNE Visualization of X_train and X_test')
plt.legend()
plt.show()
...
for epoch in range(num_epochs):
    ...
    if test_mae < best_test_mae:
        ...
        torch.save(model.state_dict(), 'best_model_mae.pth')

    if test_loss < best_test_mse:
        best_test_mse = test_loss
        best_epoch_mse = epoch
        torch.save(model.state_dict(), 'best_model_mse.pth')

    if test_r2 > best_test_r2:
        best_test_r2 = test_r2
        best_epoch_r2 = epoch
        torch.save(model.state_dict(), 'best_model_r2.pth')
...
model.load_state_dict(torch.load('best_model_r2.pth'))
model.eval()
...
for index in range(0, 200):
    ...
    plt.legend()
    plt.savefig(os.path.join('prediction-wjs-test', f'CD_{index}.png'))
    plt.close()
