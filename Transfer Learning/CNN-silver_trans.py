...
train_data = pd.read_csv('data/train_cd_silver_0420.csv')
test_data = pd.read_csv('data/test_cd_silver_0420.csv')
...
x_sequence = np.linspace(300, 700, 120)
'''
for index in range(0, 800):
    ...
    plt.ylim(-maxx - 0.2 * maxx, maxx + 0.2 * maxx)
    plt.xlabel('Wavelength Index')
    plt.ylabel('Intensity')
    plt.title(f'Actual {index} Spectra')
    plt.legend()
    plt.savefig(os.path.join('train_silver_vis', f'CD_{index}.png'))
    plt.close()
'''
'''
...
for index in range(0, 600):
    ...
    sns.lineplot(x=range(len(y_train[index])), y=y_train[index], label='Actual Spectrum', color='blue', linewidth=5, linestyle='--')
    plt.xlabel('Wavelength Index')
    plt.ylabel('Intensity')
    plt.title(f'Actual {index} Spectra')
    plt.legend()
    plt.savefig(os.path.join('test_silver_vis', f'CD_{index}.png'))
    plt.close()
...
pretrained_model.fc1 = nn.Linear(1024, 512, bias=True)
pretrained_model.fc2 = nn.Linear(512, 120, bias=True)
...
for epoch in range(num_epochs):
    ...
    if test_mae < best_test_mae:
        ...
        torch.save(pretrained_model.state_dict(), 'best_model_mae_silver.pth')

    if test_loss < best_test_mse:
        best_test_mse = test_loss
        best_epoch_mse = epoch
        torch.save(pretrained_model.state_dict(), 'best_model_mse_silver.pth')

    if test_r2 > best_test_r2:
        best_test_r2 = test_r2
        best_epoch_r2 = epoch
        torch.save(pretrained_model.state_dict(), 'best_model_r2_silver.pth')
...
pretrained_model.load_state_dict(torch.load('best_model_r2_silver.pth'))
pretrained_model.eval()
...
plt.savefig('Loss_CNN_silver.png')
plt.show()
...
for index in range(0, 200):
    ...
    ax.xaxis.set_tick_params(width=5)  # 设置刻度宽度
    ax.yaxis.set_tick_params(width=5)
    plt.savefig(os.path.join('prediction-wjs-silver', f'CD_{index}.png'))
    plt.close()
